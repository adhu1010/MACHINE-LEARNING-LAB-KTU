...
class DecisionTreeID3:
    ...
    def _add_edges(self, tree, G, parent=None, parent_attr=None):
        if isinstance(tree, dict):
            for key, value in tree.items():
                for val, subtree in value.items():
                    if parent is not None:
                        G.add_edge(parent, f'{key}={val}')
                    self._add_edges(subtree, G, parent=f'{key}={val}', parent_attr=key)
        else:
            if parent is not None:
                G.add_edge(parent, f'Predict={tree}')


file_path = '/home/cs-ai-03/adhwaith/TEST/play_tennis.csv' 
